...
namespace Modules\Cms\Http\Controllers\Admin\Includes;
...
trait Image
{
    ...
    public function imageSave(Request $request)
    {
        if ($request->hasFile('file')) {
            ...
            $galleryImage = $this->contentHelper->saveImage($inputImageData);
            $returnArray = [
                'id' => $galleryImage->id,
                'status' => true,
                'fileName' => $fileName,
    ...
    {
        $gallery = $this->contentHelper->get($request->content_id);
        $items = [];
    ...
    {
        $galleryItem = $this->contentHelper->getGalleryItem($request->id);
        if ($galleryItem->id) {
            if (Storage::disk('elegant')->delete($request->file)) {
                $this->contentHelper->deleteContentImage($request->id, $galleryItem->image_path);
            }
        }
    }
    ...
    public function dataUpdate(Request $request)
    {
        ...
        $this->contentHelper->updateContentItems($updateData);
