...
@push('script')
    <script src="{{ mix('js/admin/seo/listSeo.js') }}"></script>
@endpush

@push('style')
    <link rel="stylesheet" type="text/css" href="{{ mix('css/admin/seo/listSeo.css') }}">
@endpush

<x-layout>
    @section('toolbar')
        <x-toolbar :breadcrumbs="$breadcrumbs">
            <a href="{{ route('seo_add', ['type' => 'common']) }}" class="btn btn-primary mx-3">Add SEO</a>
        </x-toolbar>
    @endsection
    <div class="card">
                ...
                <div class="menu menu-sub menu-sub-dropdown w-300px w-md-325px kt-toolbar-filter" data-kt-menu="true"
                    ...
                    <div class="px-5 py-3">
                        <div class="mb-5">
                            ...
                            <select id="model" class="form-select form-control-solid" name="model"
                                data-kt-select2="true" data-server="true" data-placeholder="Select Category"
                                data-option-url="{{ route('seo_model_options') }}"
        ...
        <div class="card-body pt-0">
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="listSeo"
                data-url="{{ route('seo_table') }}">
                <thead>
