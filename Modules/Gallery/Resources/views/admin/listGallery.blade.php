...
@push('script')
    <script src="{{ mix('js/admin/listGallery.js') }}"></script>
@endpush

@push('style')
    <link rel="stylesheet" type="text/css" href="{{ mix('css/admin/listGallery.css') }}">
@endpush

<x-layout>
    @section('toolbar')
        <x-toolbar :breadcrumbs="$breadcrumbs">
            <a href="{{ route('gallery_add') }}" class="btn btn-primary mx-3">Add Gallery</a>
        </x-toolbar>
    @endsection
    ...
    <div class="card">
        ...
        <div class="card-body pt-0">
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="listGallery"
                data-url="{{ route('gallery_table') }}">
                <thead>
